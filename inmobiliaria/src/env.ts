...
export interface Env {
  ...
  // ── Google Calendar directo, UNA agenda POR VENDEDOR (nicho inmobiliaria:
  // agendarVisitaPropiedad, moverVisitaPropiedad, cancelarVisitaPropiedad) ──
  // Con GOOGLE_SERVICE_ACCOUNT_JSON de arriba + al menos una de estas tres,
  // esas 3 tools crean/mueven/cancelan el evento DIRECTO en la agenda del
  // vendedor asignado (Diego/Alfonso/Ismael — ver src/db/propertyVisits.ts) y
  // comprueban ahí mismo que no tenga ya una cita en ese horario, sin pasar
  // por Composio ni por ninguna hoja de cálculo. Cada calendario debe estar
  // compartido con el client_email del service account (permiso "Realizar
  // cambios en los eventos"). Sin la del vendedor asignado, esas tools
  // registran la visita local (sin evento ni chequeo de disponibilidad) y lo dicen.
  GOOGLE_CALENDAR_ID_DIEGO?: string;
  GOOGLE_CALENDAR_ID_ALFONSO?: string;
  GOOGLE_CALENDAR_ID_ISMAEL?: string;
